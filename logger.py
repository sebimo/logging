...
class Logger:
    ...
    def setup_test_experiment(self):
        print("Loggable parameters: model, lr, embedding")
...
if __name__ == "__main__":
    # Create database and start logger
    logger = Logger(database="test.db")
    
    # Create the parameters and metrics to log (can be called before all experiments)
    logger.add_parameter("test_int", Type.INT)
    logger.add_perf_metric("acc", Type.FLOAT)
    
    # You can only start experiments with already registered parameters 
    try:
        logger.start_experiment(parameters={"test_int": 1, "hello": "world"})
    except sqlite3.OperationalError:
        print("Handled unknown column")
        
    # Before logging anything the current experiment needs to be started
    try:
    ...
    # -> every logging operation afterwards will be assigned to that experiment
    for i in range(50):
